...
| **真机状态** | ✅ **手表在线的（2026-09-19 16:10 实测）**：`192.168.47.104:45165`，`list targets` 看到目标。新 HAP 已安装并重启，启动日志健康（`DsHash: selfTest=PASS` / `Boot selfTest=PASS` / `Boot deviceId ready`）。<br>★ **IP 会变（DHCP），每次都要重新问用户当前地址**；历史 IP：`192.168.47.107`、`192.168.47.104`。<br>★ 判据纪律：**手表不响应 ICMP**，`ping` 不通≠离线，但**反过来 `ping` 返回「无法访问目标主机」= 该 IP 根本不存在**，此时不要再扫端口，直接去手表上读新地址。**唯一权威判据是 `tconn`。** |
...
- ✅ **真机验证已做（2026-09-19 16:10）**，结果如下（手表在线 `192.168.47.104:45165`）：
  ...
  | 网络连通 | ✅ `ping chat.deepseek.com` 成功（57ms，0% 丢包） |
