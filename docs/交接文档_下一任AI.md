...
| 当前可装包 | `entry/build/default/outputs/default/entry-default-signed.hap`（**1,622,969 字节 / md5 `ad01d8b3acd4a3ea129b979812a01eba`，2026-09-19 14:43 构建**） |
| 已修 Bug | **18 个**（登录/输入法/发送无反应/PoW算法/SSE格式/THREAD_BLOCK_6S/渲染不刷新/布局溢出 + 09-18 的 3 个 + 本轮新增 1 个，详见第 2 节） |
| 本轮新增功能 | 右上角 `≡` → **历史对话列表页**；删除「快速/专项模式」切换；`⋯` 设置浮层只留深度思考/联网搜索；**登录页新增「微信扫码登录」入口**（见第 2.5 节与 `docs/微信扫码登录_设计与可行性.md`） |
...
| **真机状态** | ✅ **手表在线的（2026-09-19 16:10 实测）**：`<DEVICE_IP>:45165`，`list targets` 看到目标。新 HAP 已安装并重启，启动日志健康（`DsHash: selfTest=PASS` / `Boot selfTest=PASS` / `Boot deviceId ready`）。<br>★ **IP 会变（DHCP），每次都要重新问用户当前地址**；历史 IP：`<DEVICE_IP>`、`<DEVICE_IP>`。<br>★ 判据纪律：**手表不响应 ICMP**，`ping` 不通≠离线，但**反过来 `ping` 返回「无法访问目标主机」= 该 IP 根本不存在**，此时不要再扫端口，直接去手表上读新地址。**唯一权威判据是 `tconn`。** |
| 当前状态 | App 处于**未登录**（真机实测 Preferences 里只有 `auto_think`/`auto_search`/`font_scale`/`device_id`/`last_mode`/`last_session_id`，**没有 `token` 键**），**需用户在手表上登录一次**才能继续端到端验证 |
...
1. 测试脚本用 `keyEvent Back` 想收起输入法 → 输入法已在 `uiInput text` 时自动收起，
   ...
   > ★ 2026-09-19 修正：这一步的真正原因是「当时键盘已经收了，所以 Back 落到了导航上」。
   > 真机重新验证：**键盘在屏时按 Back 只会收键盘，不会弹页面**（连续 3 轮复现）。
   > 所以 Back 本身没问题，问题是**在键盘已收的情况下按了 Back**。
   > **纪律：按 Back 收键盘前，先确认键盘确实在屏上（用 `./tools/kbstate.sh`）。**
2. 紧接着的一击「点发送」坐标，落在了 `Index` 首页的「**退出登录**」上；
3. 「退出登录」**没有任何二次确认**，一点就 `Store.remove(TOKEN)` → 登录态没了。

**▲ 本次事故最根本的教训：是「连续盲点」而不是「Back 这个键」。**
**每一次点击前都要重新确认屏幕状态**（`shot` / `tree` / `kbstate.sh`），
不要用「上一步既然成功、下一步坐标应该也对」来做连续操作。
...
>
> ★★ **精确定位坐标请用布局树，不要靠肉眼看截图估**（2026-09-19 新增 `tree` 子命令）：
> ```bash
> ./tools/w.sh tree     # 打印界面布局树：类型 | 文字 | bounds(物理像素) | 是否可点/可滚
> ```
> 它直接给你 `[x1,y1][x2,y2]`，取中心即可 `tap`，**不用再猜**。
> （我本轮就是靠肉眼估坐标，把点击落到了标签栏下方，白试了好几次。）
>
> ★★★ **收键盘：用 `key Back`，这是真机验证过的正确做法**（2026-09-19 修正，之前写反了）
> ```bash
> ./tools/w.sh key Back     # 先收键盘，页面完好留在登录页
> ```
> 真机连续 3 轮验证：**Back 只会收起键盘，不会把页面弹走**。
> 此前"不要用 Back 收键盘"的告警来自 Bug 11 时期的一次事故 ——
> 那次是因为当时**本来就在从入口页新进登录页**，Back 正好退了那一步导航，
> 并非 Back 会误退页面。**结论修正：Back 收键盘是安全的。**
>
> **为什么 App 内给不出「收起键盘」按钮**（这是硬约束，别再尝试）：
> 本机输入法窗口是 **`bounds=[0,0][466,466]` 的全屏窗口**，会**完全盖住 App**：
> - 键盘在屏时 `uitest dumpLayout -b com.dswatch.round` 返回**空布局树**（children 为空）；
> - 键盘在屏时截图里**找不到 App 任何控件的像素**（我加过按钮、真机验证过、已删除）。
>
> 所以：**App 里任何"点这里收键盘"的按钮都不可能被点到**，不要白费力气。
> 用户在输入态看到的提示是「按侧键返回可收起键盘」，走的就是 Back 这条路。

### ★ 判断「界面到底变成什么样」的三条纪律（2026-09-19 踩坑，浪费了半小时）

**① 绝对不要用「截图文件字节数」判断界面变没变。**
那套自带输入法的字形是**近乎纯黑压在纯黑背景上**，JPEG 压完，
「键盘态」与「登录页」只差约 **690 字节**（14826 vs 14135），
完全淹没在压缩噪声里。我因此误判「点击没生效」，
还去动了一个**本来完全正常**的代码路径 —— 比报错更危险。

**② 要判断界面状态，用这两个可靠判据：**
```bash
./tools/kbstate.sh        # KEYBOARD(键盘全屏) / APP(App 页面在屏) —— 按布局树归属判断
./tools/w.sh tree         # 看 App 布局树里的真实节点与坐标
```
`kbstate.sh` 是本轮新增的，它读布局树的 `bundleName`：
是 `com.huawei.hmos.inputmethod` 还是 `com.dswatch.round`，一目了然。

**③ 真要比较图像，就比「像素」而不是比「文件大小」。**
用 Pillow 统计亮度/颜色团块。例如判断某按钮有没有渲染出来：

```python
from PIL import Image
im = Image.open('shots/x.jpeg').convert('RGB'); px = im.load()
found = [(x,y) for y in range(0,140) for x in range(466)
         if abs(px[x,y][0]-0x1A)<10 and abs(px[x,y][2]-0x2E)<12]
print("按钮底像素数 =", len(found))   # 0 → 按钮根本没渲染出来
```

**④ 抓帧偶发「全黑」是设备侧 framebuffer 陈旧，不是 App 的问题。**
现象：`snapshot_display` 成功、文件 4227 字节、全屏均匀纯黑，
但 `aa dump -l` 显示 App 仍是 `#FOREGROUND` 且窗口 attached。
**处理：`aa force-stop` + `aa start` 重启 App 即可恢复**，不要怀疑代码。

**⑤ `hilog tail` 计数不能当判据。** 环形缓冲区会被后续日志刷掉 ——
我一度出现「计数从 5 掉回 0」的诡异现象，就是缓冲区被冲了。
要判断键盘在不在，用上面的 `kbstate.sh`。
...
| `tools/kbstate.sh` | **新增（09-19）** | **「键盘在不在屏上」的唯一可靠判据**：按布局树的 `bundleName` 区分 `KEYBOARD`(inputmethod 全屏) / `APP`。别再用截图字节数或 `hilog` 计数判断（都不准，见 3.x 节） |
| `tools/w.sh tree` | **新增（09-19）** | **打印界面布局树**：`类型 \| 文字 \| bounds(物理像素) \| 是否可点/可滚`。**定位坐标一律用它，别靠肉眼看截图估** |
| `tools/w.sh kb` | **新增（09-19）** | 键盘事件计数（辅助参考；**不如 `kbstate.sh` 可靠**，缓冲区会被刷） |
...
>
> **★ ping 判据的准确用法（2026-09-19 修正，之前一度写反过）**：
> 要区分 ping 的**两种**失败：
> - `请求超时`（Request timed out）→ 主机可能存在但**不回 ICMP**（手表就是这种）→ **先试一次 `tconn` 再下结论**；
> - `无法访问目标主机`（Destination host unreachable）→ 本机 ARP 都拿不到 MAC，**该 IP 根本不存在**，
>   此时扫端口、反复 `tconn` **全是纯浪费**，**必须去手表上重新打开「通过 WLAN 调试」读新地址**。
>
> 一句话：**`tconn` 是唯一权威判据；`ping` 只能用来快速排除「这个 IP 压根不存在」。**

**复验步骤（等网络就绪后照做）**：

> ⚠️ **2026-09-19 14:43 现状**：手表**已离线**（`.104:45165` 失效，`list targets` = `[Empty]`；
> `ping .104` 返回「无法访问目标主机」= 该 IP 不存在）。**需用户在手表上重新打开「通过 WLAN 调试」并读新地址。**
> 本轮因此没能做真机验证 —— 已完成的是：代码编译通过 + 三套离线回归全绿 + HAP 产物已确认含新功能字节。
...
7. **微信扫码登录入口**（本轮新增）：进登录页 → 点顶部「微信扫码」→ 应看到
   **「微信扫码登录尚未开放 / 该功能需服务端支持 / 当前请使用密码登录」+ 「去密码登录」按钮**。
   ★ 若真看到的是**转圈**或**「请检查手表网络」**，那是 bug（说明降级判断没生效），不是"未配置"。
   ★ 手机上验证「微信扫码」全链路，需要先按设计文档 §6.4 部署中继并填 `RELAY_BASE`。

---

## 5.7 ✅ 用户需求三：登录页新增「微信扫码登录」（本轮新增功能）

**用户原话**：
> 在登录方式界面新增"微信扫码登录"选项，并参考 DeepSeek 网页版所采用的微信扫码登录方式适配实现。
> 请明确扫码登录的整体流程、二维码的获取与展示、扫码后的状态回调与登录态建立，
> 以及如何降低被风控识别的风险，并保证与现有登录方式的兼容。

**结论速览**：流程/契约已完整逆向（可复核）；**"手表直接显示二维码"这条路经实测否决**；
落地用的是「**手表显示配对码 → 手机网页扫码 → 手表轮询取 token**」。

| 用户要求 | 落实 |
|---|---|
| 整体流程 | 设备码流程（对齐 RFC 8628）：`start` → 显示配对码 → 手机扫码 → 手表轮询 → 取 token |
| 二维码的获取与展示 | ★ **手表端拿不到二维码**（见下"否决证据"）→ 改为**大号配对码** + 手机侧地址 |
| 扫码后的状态回调 | `QrLoginService.tick()` 状态机：`pending`/`scanned`/`ok`/`expired`/`denied`，每条都有 UI 落点 |
| 登录态建立 | 走 `AuthService.adoptToken()` → `Store.put(TOKEN/USER)`，与密码登录**同一个出口** |
| 降低风控风险 | 复用持久化 `device_id` + 同一套请求头（Bug 15 的成果）；扫码路径不碰密码，风控面更小 |
| 与现有登录兼容 | 密码登录**零改动**；顶部双段切换；跳转共用 `finishLogin()`（Bug 12 的顺序） |

**★ "手表直接显示二维码"被否决的实测证据**（细节见 `docs/微信扫码登录_设计与可行性.md`）：

1. 微信官方 `WxLogin` 组件（`wxLogin.js`，仅 1563 字节）**自己不生成二维码**，
   它只是把 `open.weixin.qq.com/connect/qrconnect?...` 塞进 **iframe**，二维码由微信服务器在 iframe 内渲染
   → **没有图片 URL 可下载**；
2. 用普通 HTTP 客户端直接请求 `qrconnect`（即便补齐 `Referer` / `login_type=jssdk` / 完整 UA），
   实测**恒返回 892 字节错误页**，错误码 `qqBS_a089916-0`
   → 微信要求请求真正来自开放平台登记、域名绑定的浏览器页面；
3. 回调换 token 的 `nonce` 由服务端在 OAuth 跳转时下发，**客户端无法自行构造**。

**本轮的落地范围（诚实口径）**：

- ✅ 已完成：客户端全流程代码（`QrLoginService.ets` / `LoginPage` 双模式 / `Constants` 契约常量 /
  `AuthService.adoptToken()`）、设计文档、编译通过、四套离线回归全绿。
- ❌ **未完成**：`RELAY_BASE` 仍是空占位符 → **中继服务端未部署** → 真机上点「微信扫码」
  会看到「尚未开放」的降级提示。**这是设计好的行为，不是 bug。**
- ✅ **真机验证已做（2026-09-19 16:10）**，结果如下（手表在线 `<DEVICE_IP>:45165`）：

  | 验证项 | 结果 |
  |---|---|
  | 新 HAP 安装 + 重启 | ✅ 成功，启动日志健康（`DsHash: selfTest=PASS` / `Boot selfTest=PASS` / `Boot deviceId ready`） |
  | 「密码登录 / 微信扫码」分段控件渲染 | ✅ 正常，蓝色药丸在「密码登录」上 |
  | 点「微信扫码」→ 降级提示 | ✅ 显示「微信扫码登录尚未开放 / 该功能需服务端支持 / 当前请使用密码登录」+「去密码登录」按钮 + 错误行「微信登录服务端尚未部署，请先用密码登录」 |
  | 点「去密码登录」 | ✅ 正确切回密码表单 |
  | 账号框输入文本 | ✅ `uitest uiInput text` 正常写入（验证后已清空） |
  | 网络连通 | ✅ `ping <API_HOST>` 成功（57ms，0% 丢包） |
  | 键盘收起方式 | ✅ **`key Back` 可收起，页面完好留在登录页**（连续 3 轮复现） |
  | 登录态 | ❌ 仍**无 token**（Preferences 里无 `token` 键）→ 需用户登录一次 |

- ⚠️ **本轮发现并修正的一个错误设计**：我曾在输入态给 LoginPage 加过一个「收起键盘」按钮，
  真机验证证明**它永远点不到**（输入法是 `[0,0][466,466]` 全屏窗口，把 App 整个盖住，
  键盘在屏时 App 窗口的布局树是**空的**）。**已删除该按钮**，改为在输入态显示
  文案「按侧键返回可收起键盘」，把正确操作告诉用户。详见第 3 节的踩坑记录。
...
- 【禁止】在真机调试里「盲点」连续坐标、或靠肉眼看截图估坐标——改用 `./tools/w.sh tree` 读布局树拿精确 `bounds`。
  （★ 2026-09-19 修正：原此处写「不要用 `keyEvent Back` 收键盘」，**该结论是错的** ——
  真机连续 3 轮验证 `Back` 只收键盘、页面完好留在登录页。Bug 11 那次弹走是因为当时本来就在做导航，不是 Back 的锅。
  **正确做法：收键盘就用 `./tools/w.sh key Back`。**）
- 【禁止】**在 App 里做「点这里收起键盘」的按钮**——这在本机**物理上不可能生效**：
  输入法窗口是 `bounds=[0,0][466,466]` 的**全屏窗口**，会把 App 整个盖住
  （键盘在屏时 App 窗口的 `dumpLayout` 返回**空布局树**，截图里也**找不到 App 任何控件的像素**）。
  **已真机验证并删除**过这个按钮，别再重新加。
  唯一途径是系统 Back，用文案告诉用户即可（现在显示「按侧键返回可收起键盘」）。
- 【禁止】**用「截图文件字节数」判断界面变没变**——深色输入法压在纯黑背景上，
  键盘态与页面态只差约 **690 字节**，淹没在 JPEG 噪声里。改用 `./tools/kbstate.sh`（布局树判据）。
  同理 **别用 `hilog tail` 计数**判断键盘状态（环形缓冲区会被刷）。
...
- 【禁止】**两种登录方式各写一份「登录成功后跳转」**——必须共用 `LoginPage.finishLogin()`。理由：`router.clear() → replaceUrl()` 这个顺序是 Bug 12 踩出来的，复制一份就一定会有一份漏改。
- 【禁止】**把 `QrLoginService.RELAY_BASE` 填成假地址**来「让它看起来能用」——未配置时功能会明确降级为「尚未开放」，这是**设计好的**；填假地址会把一个清楚的「未配置」变成一堆看不懂的网络超时。
- 【禁止】**尝试抓 `open.weixin.qq.com/connect/qrconnect` 的二维码**——已实测否决：该页要求浏览器来源 + 域名绑定，非浏览器请求恒返回 892 字节错误页（`qqBS_a089916-0`）；且二维码是微信服务器在 iframe 内渲染的，**根本没有图片 URL 可下载**。详见 `docs/微信扫码登录_设计与可行性.md` §3。
- 【注意】扫码登录只是**多一个登录入口**，产出的 token 与密码登录**完全等价**；落盘必须走 `AuthService.adoptToken()` 这**一个**出口，不要在扫码路径里另写一份 `Store.put`。
