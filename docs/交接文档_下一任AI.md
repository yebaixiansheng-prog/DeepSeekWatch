...
截至 **2026-09-18 20:45**，本轮修复**只做了代码层 + 离线回归验证**，
**真机端到端没有复验**，原因是电脑与手表不在同一网段：

| 项 | 值 |
|---|---|
| 电脑 IPv4 | `192.168.31.20/24`（另有一个 `172.30.200.33` 的虚拟适配器 `vgate0`） |
| 手表报的地址 | `192.168.47.107:45165`（**两次截图都是这个地址**，IP 段与电脑完全不同） |
| `hdc list targets` | `[Empty]` |
| 主动扫描 | `192.168.31.x` / `192.168.47.x` / `192.168.1.x` / `192.168.0.x` / `192.168.100.x` / `172.30.200.x` 的 **45165 / 4516 / 5555 端口共 4572 个地址全无响应** |

> ⚠️ **不要在网段不一致时反复扫描**（已扫 4572 个地址，全部无效，纯浪费时间）。
> 手表"通过 WLAN 调试"页显示的 IP 是**手表自己所在的网段**；电脑在 `192.168.31.x`
> 而手表在 `192.168.47.x`，说明两者**连的不是同一个 Wi-Fi**（手表可能连的是手机热点，
> 或路由器开了访客网络/双频不同网段）。
> **唯一解法**：把电脑和手表连到**同一个 Wi-Fi**，然后重读手表页面上的地址。
> 如果路由器的 2.4G/5G 分了不同网段，两端必须连同一个频段。

**若确认在同一网段但仍连不上**，按顺序排查：
1. 手表"通过 WLAN 调试"开关**是否还开着**（息屏/重启后会自动关闭）；
2. 电脑防火墙是否拦了 45165 出站；
3. 路由器是否开了 **AP 隔离**（客户端互访被禁）——这是同网段连不上最常见的原因。
